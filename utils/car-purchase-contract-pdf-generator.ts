...
export class CarPurchaseContractPDFGenerator {
    /**
     * Generate PDF from car purchase contract data using server-side Puppeteer
     */
    static async generateFromContract(contract: CarContract, options: PDFGeneratorOptions = {}): Promise<void> {
        const { filename = `car-purchase-contract-${contract.carPlateNumber}-${new Date().toISOString().split('T')[0]}.pdf`, language = 'en', format = 'A4', orientation = 'portrait' } = options;
        ...
        try {
            ...
            const response = await fetch('/api/generate-car-purchase-contract-pdf', {
                method: 'POST',
                headers: {
            ...
            console.error('Car purchase contract PDF generation failed:', error);
            throw error;
        }
    }
    ...
    private static async generateContractHTML(contract: CarContract, language: string): Promise<string> {
        ...
        return JSON.stringify({
            ...
            language,
            contractType: 'car-purchase', // This is the key difference
        });
    }
}
