 ...
 * }
 *
 * TODO: Integrate with 019SMS or your provider API when you have credentials
 * TODO: Integrate with 019SMS or your provider API when you have credentials
 */
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('[send] phone_numbers count:', numbers.length, numbers.slice(0, 3));
        ...
        if (numbers.length > 0) {
            ...
            console.log('[send] SMS result:', result.success ? 'OK' : 'FAIL', result.error || result.message);
            if (result.success) {
                status = 'sent';
            } else {
                status = 'failed';
            ...
            console.log('[send] No phone_numbers in request - skipping SMS');
        }
