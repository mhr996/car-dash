        ...
        const fetchCar = async () => {
            try {
                ...
                if (data) {
                    ...
                    // Set car source based on existing data
                    if (data.source_type === 'broker') {
                        setCarSource('brokerage');
                    } else if (data.source_type) {
                        setCarSource(data.source_type);
                    } else {
                        setCarSource(data.provider ? 'provider' : 'customer');
                    }

                    if (data.source_type === 'brokerage' || data.source_type === 'broker' || data.source_type === 'customer') {
                        if (data.customers) setSelectedCustomer(data.customers);
                    } else if (data.customers && !data.provider) {
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <CarSourceFields
                                    ...
                                    onProviderChange={handleInputChange as any}
