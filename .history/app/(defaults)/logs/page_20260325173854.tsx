...
const LogsPage = () => {
    ...
    const getCarModelDetails = (log: Log) => {
        ...
        const car = log.car as Log['car'] & {
            ...
            exchange_returned_to_showroom?: boolean;
            deal_cancelled_showroom_restored?: boolean;
            deal_cancelled?: boolean;
        };
        const showReturned = car.returned_to_customer === true || car.status === 'returned_to_customer';
        const showShowroomRestored = car.exchange_returned_to_showroom === true || car.deal_cancelled_showroom_restored === true;
        ...
        return (
            <div className="text-sm">
                ...
                {showReturned && <div className="mt-1 text-xs font-medium text-danger">{t('returned_to_customer')}</div>}
                {showShowroomRestored && <div className="mt-1 text-xs font-medium text-danger">{t('log_car_status_back_to_showroom')}</div>}
            </div>
        );
    };
