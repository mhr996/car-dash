...
const EditDeal = ({ params }: { params: { id: string } }) => {
    ...
    const handleCancelDealConfirm = async () => {
        ...
        try {
            ...
            // Keep log rows; update embedded deal to cancelled (exchange also patched server-side in side-effects)
            if (!isExchange) {
                try {
                    ...
                    if (!logRes.ok) {
                        await patchDealCancelledInLogs(
                            supabase as Parameters<typeof patchDealCancelledInLogs>[0],
                            String(dealId),
                            cancelReason.trim(),
                        );
                    }
                } catch (logErr) {
                    console.warn('Failed to patch activity logs after cancel:', logErr);
                }
            }
            ...
            if (isExchange) {
                try {
                    const { data: sessionData } = await supabase.auth.getSession();
                    const token = sessionData.session?.access_token;
                    const res = await fetch('/api/deals/exchange-cancel-side-effects', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            ...(token ? { Authorization: `Bearer ${token}` } : {}),
                    ...
                    console.error('Exchange cancellation side effects failed:', exchangeErr);
                    try {
                        await applyExchangeDealCancellationSideEffects({
                            dealId: String(dealId),
                            customerCarId: exchangeCustomerCarId,
                        ...
                        setAlert({
                            message: t('deal_cancelled_exchange_cleanup_warning'),
                            type: 'danger',
                        });
                    }
                }
        ...
        <div className="container mx-auto p-6">
            ...
            <div className="panel">
                <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="panel">
                                <div className="mb-5 flex items-center gap-3">
                                    ...
                                    <h5 className="text-lg font-semibold dark:text-white-light">
                                        {existingAttachments && existingAttachments.length > 0 ? t('add_more_attachments') : t('deal_attachments')}
                                    </h5>
                                </div>
                                                                        ...
                                                                        <div className="grid grid-cols-4 gap-4 mb-3 py-2">
                                                                            ...
                                                                            <div className="text-center">
                                                                                <span className="text-sm text-gray-700 dark:text-gray-300">
                                                                                    {formatCurrency(parseFloat(form.selling_price || '0'))}
                                                                                </span>
                                                                            </div>
                                                                            <div className="text-center">
                                                                                <span className="text-sm text-gray-700 dark:text-gray-300">1</span>
                    ...
                    {/* Submit / attachments; cancel deal allowed for completed deals too */}
