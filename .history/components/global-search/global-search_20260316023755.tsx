...
const GlobalSearch = () => {
    ...
    const performSearch = useCallback(async (query: string) => {
        ...
        if (!trimmed || (trimmed.length < 2 && isNaN(Number(trimmed)))) {
            setSearchResults([]);
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (uniqueCustomers.length > 0) {
                ...
                uniqueCustomers.forEach((customer) => {
                    results.push({
                        ...
                        type: 'customer',
                        title: customer.name || t('unnamed_customer'),
            ...
            if (deals) {
                deals.forEach((deal: any) => {
                    ...
                    results.push({
                        id: deal.id.toString(),
                        type: 'deal',
                        title: deal.title || t(`deal_type_${deal.deal_type}`),
            ...
            // Search bills - include bill_payments data and tranzila_document_number
            const billQueries = await Promise.all([
                // Search bills table - customer, car_details, bill_type, tranzila_document_number
                supabase
                    .from('bills')
                    .select('id, customer_name, bill_type, total_with_tax, total, status, car_details, tranzila_document_number')
                    .or(`customer_name.ilike.%${query}%, bill_type.ilike.%${query}%, car_details.ilike.%${query}%, tranzila_document_number.ilike.%${query}%`)
                    .limit(10),

                // For numeric query: search by bill id or exact tranzila_document_number
                ...(trimmed.length >= 1 && !isNaN(Number(trimmed))
                    ? [
                          supabase.from('bills').select('id, customer_name, bill_type, total_with_tax, total, status, car_details, tranzila_document_number').eq('id', Number(trimmed)).limit(1),
                          supabase.from('bills').select('id, customer_name, bill_type, total_with_tax, total, status, car_details, tranzila_document_number').eq('tranzila_document_number', trimmed).limit(5),
                      ]
                    : []),
            ...
            const allBills: any[] = [];
            const paymentQueryResult = billQueries[billQueries.length - 1];

            // First element: direct bills (or/ilike search)
            const firstBills = billQueries[0].data;
            if (Array.isArray(firstBills)) allBills.push(...firstBills);

            // Middle elements (when numeric): bills by id or tranzila_document_number
            for (let i = 1; i < billQueries.length - 1; i++) {
                const midBills = billQueries[i].data;
                if (Array.isArray(midBills)) allBills.push(...midBills);
            }

            // Last element: bills from bill_payments
            if (Array.isArray(paymentQueryResult.data)) {
                paymentQueryResult.data.forEach((payment: any) => {
                    if (payment.bills) {
            ...
            if (uniqueBills.length > 0) {
                uniqueBills.forEach((bill: any) => {
                    ...
                    const billTypeLabel = bill.bill_type ? t(`bill_type_${bill.bill_type}`) : t('bill');
                    results.push({
                        id: bill.id.toString(),
                        type: 'bill',
                        title: `${billTypeLabel} - ${bill.customer_name || t('unknown_customer')}${matchInfo}`,
