...
const CommissionFilters: React.FC<FilterProps> = ({ onFilterChange, onClearFilters }) => {
    const { t } = getTranslation();
    ...
    const handleInputChange = (field: keyof CommissionFilters, value: string) => {
    ...
    const providerOptions = [{ value: '', label: t('all') }, ...providers.map((p) => ({ value: p.id, label: p.name }))];
                ...
                <div className="panel border-white-light px-5 py-4 dark:border-[#1b2e4b]">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        <div className="space-y-2">
                            ...
                            <CustomSelect options={providerOptions} value={filters.providerId} onChange={(v) => handleInputChange('providerId', v)} placeholder={t('all')} className="form-select" />
                        </div>
                        <div className="space-y-2">
